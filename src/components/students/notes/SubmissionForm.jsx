...
import { Camera, X, Upload, Send, CheckCircle, AlertCircle, Image } from "lucide-react";
...
function ImageThumb({ src, index, onRemove }) {
  return (
    <div className="relative group aspect-square rounded-xl overflow-hidden shrink-0"
...
function AddImageButton({ onFiles, disabled }) {
  ...
  return (
    <div
      className="flex gap-2 aspect-square rounded-xl items-center justify-center shrink-0 flex-col"
      style={{
        width: 72, height: 72,
    ...
    >
      ...
      >
        ...
        <span className="text-[8px] font-bold" style={{ color: "rgba(255,255,255,0.25)" }}>
          Gallery
        </span>
      </button>
...
export default function SubmissionForm({ exercise, student, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await createFormData("exercise-submissions/", formData);
    ...
    return (
      <div className="py-8 flex flex-col items-center gap-3 text-center">
        ...
        <p className="text-sm" style={{ color: "rgba(255,255,255,0.45)" }}>
          Your response has been sent to your teacher for review.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {canAddMore && (
            <AddImageButton onFiles={handleFiles} disabled={submitting} />
          )}
        </div>

        {!images.length && (
          <p className="text-xs mt-2" style={{ color: "rgba(255,255,255,0.25)" }}>
            Add at least one photo of your answer (max {MAX_IMAGES})
          </p>
        )}
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          disabled={submitting}
          rows={3}
          placeholder="Add any extra notes or context for your teacher…"
